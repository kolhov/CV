...
<template>
  <main class="flex sm:flex-row flex-col mt-8 m-x-5 w-full justify-center">
    ...
    <div class="flex flex-col m-2 p-2" v-if="locale">
      <section id="intro" class="flex flex-col items-center mb-8 pt-8 justify-center">
        <h1 class="text-xl">{{ locale.name }}</h1>
        <a :href="`/files/CV_${currLang.toUpperCase()}.pdf`" download>
          <Button
            class="mt-6 cursor-pointer font-semibold bg-green-400 hover:bg-green-300 shadow-md shadow-green-400/50 hover:shadow-green-300/50"
          >
      ...
      <section id="work" class="mb-12 max-w-148">
        ...
        <div class="flex max-w-150 w-full pb-6" v-for="job in locale.jobs" :key="job.months">
          ...
          <div class="flex flex-col w-3/4">
            ...
            <ul v-if="job.achievements.length > 1">
              <li
                class="flex flex-row py-0.5 text-sm text-muted-foreground"
                v-for="x in job.achievements"
                :key="x[0]"
              >
      ...
      <section id="education" class="mb-14 max-w-148">
        ...
        <div class="flex">
          <div class="flex flex-col relative w-full gap-1">
            <img
              src="/img/logo_web_fpf.png"
              alt="Logo FPF SLU"
              class="w-[55%] place-self-center mb-6 sm:w-[25%] sm:absolute sm:top-0 sm:right-0"
            />
        ...
        <div>
          <Accordion type="single" class="w-full mt-4" collapsible default-value="item-1">
            <AccordionItem value="item-1">
              <AccordionTrigger class="py-2 cursor-pointer"
                ><h3>{{ locale.page.courses }}</h3></AccordionTrigger
              >
              <AccordionContent class="pl-2">
                <ul>
                  <li
                    class="flex flex-row py-0.5 text-sm text-muted-foreground"
                    v-for="(x, i) in locale.courses"
                    :key="i"
                  >
      ...
      <section id="contacts" class="h-180">
        ...
        <a
          class="flex flex-row hover:underline m-2 w-1/2 pt-4"
          href="https://t.me/faantanka"
          target="_blank"
        >
