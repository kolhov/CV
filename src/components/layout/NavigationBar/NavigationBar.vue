...
function scrollCalc() {
  const mainNavLinks = document.querySelectorAll('.xx a')
  const offsetPx = 24
  ...
  mainNavLinks.forEach((link) => {
    ...
    const el = (section as HTMLElement).getBoundingClientRect()
    if (el.top <= window.innerHeight && el.bottom > offsetPx) {
      anchor.classList.add('current')
      const allCurrents = document.querySelectorAll('.current')
      if (!allCurrents?.length) anchor.classList.add('current')
...
<template>
  <div class="min-w-48">
    <div
      class="flex flex-col gap-1 sm:fixed inset-x-0 sm:left-auto sm:right-auto sm:top-20 xx items-center sm:items-start"
      v-if="locale"
      role="navigation"
    >
      <LangButton />
      ...
      <a :href="locale.github" target="_blank" class="contact">{{
        locale.github.split('://')[1]
      }}</a>
    </div>
  </div>
</template>
